...
import { getMailResult } from "../graphql/queries";
import { updateMailResult } from "../graphql/mutations";
const client = generateClient();
export default function MailResultUpdateForm(props) {
  const {
    id: idProp,
    mailResult: mailResultModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = mailResultRecord
      ? { ...initialValues, ...mailResultRecord }
      : initialValues;
  ...
  const [mailResultRecord, setMailResultRecord] =
    React.useState(mailResultModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getMailResult.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMailResult
        : mailResultModelProp;
      setMailResultRecord(record);
    };
    queryData();
  }, [idProp, mailResultModelProp]);
  React.useEffect(resetStateValues, [mailResultRecord]);
  const validations = {
    from: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateMailResult.replaceAll("__typename", ""),
            variables: {
              input: {
                id: mailResultRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MailResultUpdateForm")}
      {...rest}
    >
      <TextField
        label="From"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mailResultModelProp)}
