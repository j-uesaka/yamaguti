...
import { getReportMaster } from "../graphql/queries";
import { updateReportMaster } from "../graphql/mutations";
const client = generateClient();
export default function ReportMasterUpdateForm(props) {
  const {
    id: idProp,
    reportMaster: reportMasterModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = reportMasterRecord
      ? { ...initialValues, ...reportMasterRecord }
      : initialValues;
    setType1(cleanValues.type1);
    setType2(cleanValues.type2);
    setTemplate_report_title(cleanValues.template_report_title);
    setTemplate_text(cleanValues.template_text);
  ...
  const [reportMasterRecord, setReportMasterRecord] = React.useState(
    reportMasterModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getReportMaster.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getReportMaster
        : reportMasterModelProp;
      setReportMasterRecord(record);
    };
    queryData();
  }, [idProp, reportMasterModelProp]);
  React.useEffect(resetStateValues, [reportMasterRecord]);
  const validations = {
    type1: [{ type: "Required" }],
    type2: [],
    template_report_title: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          type1,
          type2: type2 ?? null,
          template_report_title: template_report_title ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateReportMaster.replaceAll("__typename", ""),
            variables: {
              input: {
                id: reportMasterRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ReportMasterUpdateForm")}
      {...rest}
    >
      <TextField
        label="Type1"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || reportMasterModelProp)}
