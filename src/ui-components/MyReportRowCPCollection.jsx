...
import MyReportRowCP from "./MyReportRowCP";
...
export default function MyReportRowCPCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listReports.replaceAll("__typename", ""),
          variables,
        })
      ).data.listReports;
  ...
  return (
    <div>
      <Collection
        type="list"
        searchPlaceholder="検索してね"
        direction="column"
        justifyContent="left"
        itemsPerPage={pageSize}
        isPaginated={!isApiPagination && isPaginated}
        items={itemsProp || (loading ? new Array(pageSize).fill({}) : items)}
        {...getOverrideProps(overrides, "MyReportRowCPCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
